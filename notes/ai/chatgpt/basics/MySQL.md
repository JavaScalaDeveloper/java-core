...
- 结点结构：B-Tree 的每个结点既可存键也可存数据（或数据指针）；B+Tree 的内部结点只存键和子结点指针，真实数据（或主键指向）只在叶子结点。

- 叶子结点：B-Tree 的非叶结点也可能挂数据；B+Tree 所有记录都在叶子层，查询路径长度更稳定。

- 结点扇出：B+Tree 内部结点不放数据，同等页大小下能放下更多键，扇出更大、树更矮，磁盘 IO 更少。

- 叶子链接：B+Tree 叶子结点按键序用双向链表串起来，范围扫描可顺序扫叶子；B-Tree 范围查往往要反复中序遍历，随机 IO 更多。

- 查询性能：等值查询两者都能做；范围、排序、覆盖索引扫描上 B+Tree 明显更优，这也是 InnoDB 选 B+Tree 的主要原因。

- 应用场景：现代关系库聚簇/二级索引几乎都用 B+Tree（如 InnoDB）；经典 B-Tree 更多出现在部分文件系统、内存索引等场景。

# InnoDB 使用 B+树有什么优点？
InnoDB 的聚簇索引和二级索引都基于 B+Tree（按页组织，默认 16KB），主要优点：

- **树更矮、IO 更少**：内部结点只存键和指针，一页能装更多索引项，同等数据量下高度通常只有 2～4 层，一次主键点查只需几次页读取。

- **范围查询友好**：叶子按主键/索引键有序，且叶子之间有链表，`BETWEEN`、`>`、`ORDER BY`、分组扫描都能顺着叶子扫，少做随机 IO。

- **查询路径稳定**：数据只在叶子，不同键的查找深度接近，性能更可预期（对比 B-Tree 有的记录在上层、有的在下层）。

- **适合磁盘与缓冲池**：以页为单位读写，和 Buffer Pool、预读、刷脏等机制契合；页内记录再按槽位有序，利于二分查找。

- **利于覆盖索引**：二级索引叶子存「索引列 + 主键」，很多查询只扫二级索引叶子即可，不必回表。

- **插入相对可控**：页分裂/合并虽有开销，但配合页目录、变更缓冲（Change Buffer，非唯一二级索引）等，随机插入也能较好摊销。

注意：B+Tree 对「最左前缀、范围条件后列失效、函数破坏索引」等规则仍然适用；选错索引或字段选择性差时，再好的树结构也帮不上忙。
...
| 对比点 | B-Tree | B+Tree |
|--------|--------|--------|
| 数据存放 | 内部结点和叶子都可能存数据 | 只有叶子存完整数据/记录指针 |
| 内部结点 | 键 + 子指针 +（可能的）数据 | 只存键 + 子指针，扇出更大 |
| 树高 | 同样数据量通常更高或相当 | 通常更矮，磁盘 IO 更少 |
| 叶子链接 | 一般无兄弟链表 | 叶子双向链表，便于范围扫 |
| 等值查 | 可能在非叶就命中提前返回 | 必须走到叶子，路径长度稳定 |
| 范围查 | 需中序遍历，随机 IO 多 | 定位起点后顺序扫叶子即可 |
| 典型场景 | 部分文件系统、内存结构 | InnoDB / 多数关系库磁盘索引 |

补充说明：

- **为什么库用 B+Tree**：数据库索引主要瓶颈是磁盘/页 IO，B+Tree 用「矮树 + 叶子链表」同时优化点查和范围查。
- **InnoDB 里的形态**：聚簇索引叶子是整行；二级索引叶子是「二级键 + 主键」，回表再走一次聚簇索引。
- **不要和「B-Tree 索引」口语混淆**：MySQL 文档里常说的 “B-Tree 索引”，实际引擎实现一般是 **B+Tree**。
...
- 分布式架构：原生分布式，数据按分区（Partition/Tablet）打散到多节点，水平扩展不必像单机 MySQL 那样先做业务分库分表。

- 高性能和高并发：多节点并行执行、存储与计算优化，适合大规模数据与高吞吐场景。

- 强一致性和可靠性：多副本 + Paxos/类共识协议，多数派确认后提交，RPO 可接近 0；支持分布式事务与 MVCC。

- 自动负载均衡和故障切换：副本迁移、切主、负载再均衡由系统完成，应用侧少做主从切换脚本。

- 全局索引和分布式查询优化：支持全局索引与跨分区 SQL 优化，降低「分片后 SQL 难写」的成本。

- 多租户与存储优化：多租户资源隔离；压缩、编码等降低存储与 IO 成本。

- MySQL 兼容：高度兼容 MySQL 协议与语法，迁移成本相对可控（仍需验证方言、函数、锁行为差异）。

# OceanBase 的架构是怎样的？对比 MySQL 有哪些优点？
**架构（简化）：**

- **OBServer**：既承担 SQL 计算，也存本机分区数据（shared-nothing），可水平扩容。
- **RootService / 元数据服务**：集群成员、分区位置、负载与 DDL 等元信息管理（常部署在部分 Observer 上）。
- **分区与多副本**：表按分区切分；每个分区多副本，用共识协议选主与同步日志。
- **事务**：支持单机与分布式事务（两阶段/优化协议），对应用尽量表现为「一个逻辑库」。
- **多租户**：一个集群可跑多个租户，资源配额隔离。

**相对单机 MySQL 的优点：**

| 维度 | 单机 MySQL（+ 自建主从/分库） | OceanBase |
|------|------------------------------|-----------|
| 扩展 | 垂直扩容为主；水平靠分库分表 | 加节点扩容，分区自动分布 |
| 高可用 | 主从切换、半同步等自建 | 多副本共识，自动切主 |
| 一致性 | 主从异步可能丢数据 | 多数派提交，强一致更易做 |
| 分片透明 | 中间件/业务改造多 | SQL 层尽量屏蔽分片 |
| 运维 | 复制、备份、扩缩容链路长 | 集群内统一调度与均衡 |

适用：超大库、强一致金融级、希望少改业务的分布式替换。小流量、强依赖单机特性/插件时，MySQL 仍更轻。

# TiDB 的架构是怎样的？对比 MySQL 有哪些优点？
**架构（计算存储分离）：**

- **TiDB Server**：无状态 SQL 层，兼容 MySQL 协议；解析、优化、分布式执行。可水平扩多实例。
- **TiKV**：有状态 KV 存储，Region 分片，**Raft** 多副本；行存，负责 OLTP。
- **PD（Placement Driver）**：集群大脑——Region 调度、负载均衡、分配 TSO（全局时间戳，支撑分布式事务）。
- **TiFlash（可选）**：列存副本，服务分析查询；与 TiKV 通过 Raft learner 同步，形成 **HTAP**。
- **TiCDC / BR 等**：增量同步、备份恢复生态。

数据流简述：客户端 → TiDB → 向 PD 取元数据/TSO → 读写 TiKV（或分析走 TiFlash）。

**相对单机 MySQL 的优点：**

| 维度 | MySQL | TiDB |
|------|-------|------|
| 扩展方式 | 分库分表或中间件 | 存算分离，TiDB/TiKV 分别扩 |
| 分片 | 业务/中间件感知 | Region 自动分裂合并，对 SQL 更透明 |
| HA | 主从/MGR/Orchestrator 等 | Raft 自动选主，无传统主从延迟那套 |
| 事务 | 单机事务成熟；跨库难 | 分布式事务 + Percolator/乐观锁模型 |
| HTAP | 通常 OLTP/OLAP 拆库 | 可选 TiFlash，同一份数据行+列 |
| 运维心智 | 熟悉生态、插件丰富 | 组件更多（PD/TiKV），但省分库运维 |

**OceanBase vs TiDB（相对 MySQL 时怎么选，面试常问）：**

- OceanBase：存算一体、多租户、金融级强一致与 MySQL/Oracle 兼容路线成熟。
- TiDB：存算分离、云原生调度清晰，TiFlash 做实时分析更顺手，开源社区与云厂商生态活跃。
- 二者都解决「MySQL 单机容量/写入瓶颈 + 分库分表痛苦」；选型看一致性要求、HTAP、运维团队栈、兼容性验证结果。
