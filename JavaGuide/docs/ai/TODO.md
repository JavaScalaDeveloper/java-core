...
| 内容                                           | 状态                                          |
| ---------------------------------------------- | --------------------------------------------- |
| `llm-basis/llm评测.md`                  | 已完成，已进入大模型基础 README 和顶层 README |
| `system-design/llm网关.md`                 | 已完成，已进入系统设计 README 和顶层 README   |
| `agent/工作流图循环.md`                 | 已进入 Agent README、顶层 README 和面试题     |
| `system-design/ai应用架构.md` | 已进入系统设计 README、顶层 README 和面试题   |
| `system-design/ai语音.md`                    | 已进入系统设计 README、顶层 README 和面试题   |
...
| `agent/tool-calling.md`             | Agent 工具调用详解：Function Calling、MCP Tool 与权限控制 | 串起 `结构化输出与函数调用.md`、`什么是 Model Context Protocol (MCP)？和 Function Calling、Agent 什么关系？.md` 和 `ai应用架构.md`，重点讲工具 Schema、参数校验、权限审批、执行结果回传和失败恢复 |
...
| `ai-workflow-framework.md` | LangGraph / Spring AI Alibaba Graph：AI Workflow、Graph、Loop 如何落地 | 第三篇，与 工作流图循环.md 互相引用 |
...
2. `system-design/ai-observability.md`：能和 `harness工程.md`、`rag优化.md`、`llm评测.md` 接上，形成“调试 -> 评测 -> 观测”闭环。
