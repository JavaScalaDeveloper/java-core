...
1. [AI 应用系统设计](./ai应用架构.md)：先看 Prompt Demo 进入生产链路时，需要补哪些后端能力。
2. [大模型网关详解](./llm网关.md)：再看模型调用治理，多模型路由、fallback、限流和成本归因怎么放在网关层处理。
3. [AI 语音技术详解](./ai语音.md)：最后看语音 Agent，从 VAD、ASR 到 LLM、TTS、播放和打断处理。

## 核心文章

- [AI 应用系统设计](./ai应用架构.md)：从 Prompt 管理讲到模型网关、RAG、Memory、Tool 调用、异步任务、可观测、评测和安全合规，适合作为系统设计主线。
- [大模型网关详解](./llm网关.md)：说明 LLM Gateway 在模型路由、fallback、限流配额、Token 预算、成本归因、观测审计和缓存策略中的职责。
- [AI 语音技术详解](./ai语音.md)：沿语音系统链路展开 VAD、ASR、LLM、TTS、流式播放、打断处理与端云混合选型。
