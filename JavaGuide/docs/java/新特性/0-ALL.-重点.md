...
![](https://oss.javaguide.cn/github/javaguide/java/新特性/jdk8~jdk24.png)
...
![](https://oss.javaguide.cn/github/javaguide/java/新特性/2021042717491413.png)
...
![ JDK 8 到 JDK 25 每个版本的更新带来的新特性数量和更新时间](https://oss.javaguide.cn/github/javaguide/java/新特性/jdk8~jdk24.png)
...
- Java 10 Features and Enhancements : <https://howtodoinjava.com/Java 10 新特性概览/java10-features/>
...
![Oracle 官方给出的 Oracle JDK 支持的时间线](https://oss.javaguide.cn/github/javaguide/java/新特性/4c1611fad59449edbbd6e233690e9fa7.png)
...
Shenandoah 是由 Red Hat 主导开发的低暂停垃圾收集器，目标是在堆较大时仍尽量保持较短且与堆大小相对独立的暂停时间。
...
`yield` 和 `return` 的区别在于：`return` 结束当前方法并返回结果，而 `yield` 结束当前 `switch` 表达式分支并产生该表达式的值。`switch` 表达式必须覆盖所有可能的输入，但在已经穷尽所有取值时不一定需要 `default` 分支。
...
`mismatch()` 方法用于比较两个文件，并返回第一个不匹配字节的位置，如果文件相同则返回 `-1L`。
...
以 `switch` 表达式为例，它从 Java 12 开始预览，在 Java 13 中继续调整，并在 Java 14 中转为正式语言特性。转正后其语法和语义属于 Java SE 规范的一部分，不再需要启用预览功能。
...
在 [Java 18 新特性概览](./Java 18 新特性概览.md) 中，我有详细介绍到向量 API，这里就不再做额外的介绍了。
...
仔细观察原始类型的包装类（比如：`java.lang.Integer`、`java.lang.Double`），不难发现，其构造函数上都已经标记有 `@Deprecated(since="9", forRemoval = true)` 注解，这意味着这些构造函数计划在未来移除，不应该在程序中继续使用诸如 `new Integer(10)` 这样的编码方式（建议使用 `Integer a = 10` 或者 `Integer.valueOf()` 方法），如果继续使用，编译期将会产生 `'Integer(int)' is deprecated and marked for removal` 警告。并且，这些包装类型已经同 `java.util.Optional` 和 `java.time.LocalDateTime` 一样，被指定为基于值的类（value-based class），不要与 Project Valhalla 中的值类型混淆。

其次，在 `synchronized` 同步块中使用基于值的类实例，JDK 16 的编译器会产生警告。HotSpot 还可以通过诊断参数在运行期记录或阻止这类同步。即使没有启用运行期诊断，也不应该将基于值的类实例用作锁，例如：
...
`Integer` 是不可变对象，`count++` 会经过拆箱、加一和重新装箱，并将字段引用改为另一个 `Integer` 实例。因此，不同线程可能锁住不同的对象，无法为该复合操作提供稳定的互斥。如果需要并发自增，应使用固定的锁对象或 `AtomicInteger`。
...
Java 14（[JEP 370](https://openjdk.org/jeps/370)）第一次孵化外部内存访问 API，Java 15 中进行了第二次孵化（[JEP 383](https://openjdk.org/jeps/383)），在 Java 16 中进行了第三次孵化。
...
- 可用：对于需要访问外部内存的程序，该 API 应提供一套足以替代 `sun.misc.Unsafe` 的易用方案。
...
在 [Java 14 & 15 新特性概览](./Java 14 & 15 新特性概览.md) 中，我有详细介绍到密封类，这里就不再做额外的介绍了。
...
![](https://oss.javaguide.cn/github/javaguide/java/新特性/4c1611fad59449edbbd6e233690e9fa7.png)
...
> [PRNG](https://ctf-wiki.org/crypto/streamcipher/prng/项目介绍/) 用来生成接近于绝对随机数序列的数字序列。一般来说，PRNG 会依赖于一个初始值，也称为种子，来生成对应的伪随机数序列。只要种子确定了，PRNG 所生成的随机数就是完全确定的，因此其生成的随机数序列并不是真正随机的。
...
在 [Java 19 新特性概览](./Java 19 新特性概览.md) 中，我有详细介绍到外部函数和内存 API，这里就不再做额外的介绍了。
...
![新老实现的反射性能基准测试结果](https://oss.javaguide.cn/github/javaguide/java/新特性/JEP416Benchmark.png)
...
虚拟线程（Virtual Thread）是 JDK 而不是 OS 实现的轻量级线程，许多虚拟线程共享同一个操作系统线程，虚拟线程的数量可以远大于操作系统线程的数量。

虚拟线程在其他多线程语言中已经被证实是十分有用的，比如 Go 中的 Goroutine、Erlang 中的进程。

虚拟线程通常不需要为每个任务创建或切换一个操作系统线程，从而降低大量阻塞任务带来的线程资源和调度开销，并减少编写、维护和观察高吞吐量并发应用程序的工作量。
...
结构化并发的基本 API 是[`StructuredTaskScope`](https://download.java.net/java/early_access/loom/docs/api/jdk.incubator.concurrent/jdk/incubator/并发/StructuredTaskScope.html)。`StructuredTaskScope` 支持将任务拆分为多个并发子任务，在它们自己的线程中执行，并且子任务必须在主任务继续之前完成。
...
记录模式可以避免不必要的类型转换，使代码更简洁易读。记录模式本身并不会消除所有 `null` 或 `NullPointerException` 风险：`null` 不匹配记录模式，记录组件引用也仍然可能为 `null`。
...
**注意**：不要把记录模式和 [JDK16](./Java 16 新特性概览.md) 正式引入的记录类搞混了。
...
虚拟线程（Virtual Thread）是 JDK 而不是 OS 实现的轻量级线程，由 JDK 调度。许多虚拟线程共享同一个操作系统线程，虚拟线程的数量可以远大于操作系统线程的数量。
...
相比较于平台线程来说，虚拟线程是廉价且轻量级的，使用完后即可销毁，因此它们不需要被重用或池化，每个任务可以有自己专属的虚拟线程来运行。虚拟线程的暂停和恢复通常不需要为每个任务创建或切换一个操作系统线程，从而降低大量阻塞任务带来的线程资源和调度开销。
...
- [虚拟线程极简入门](https://javaguide.cn/java/并发/virtual-thread.html)
- [Java19 正式 GA！看虚拟线程如何大幅提高系统吞吐量](https://mp.weixin.qq.com/s/yyApBXxpXxVwttr01Hld6Q)
- [虚拟线程 - VirtualThread 源码透视](https://www.cnblogs.com/throwable/p/16758997.html)
...
[Java 20 新特性概览](./Java 20 新特性概览.md)已经详细介绍过记录模式，这里就不重复了。
...
[Java 20 新特性概览](./Java 20 新特性概览.md)已经详细介绍过虚拟线程，这里就不重复了。
...
JDK 23 和 JDK 22 一样，都是非 LTS（长期支持）版本，Oracle 通常仅为这类版本提供六个月的更新。它们之后的长期支持版是 2025 年 9 月发布的 JDK 25。
...
![](https://oss.javaguide.cn/github/javaguide/java/新特性/jdk22-new-features.png)
...
![Markdown 文档注释](https://oss.javaguide.cn/github/javaguide/java/新特性/jep467-markdown-documentation-comments.png)
...
`MemorySegment` 是 Foreign Function & Memory API（外部函数和内存 API）的核心类型之一，用于安全访问堆内或堆外内存；`VarHandle` 是独立的标准 API，可用于访问字段、数组元素或按内存布局访问数据。Foreign Function & Memory API 在 JDK 22 中转为正式特性。
...
Java 19 以孵化器 API 的形式引入了结构化并发。JDK 23 中该 API 处于第三次预览阶段，目的是简化多线程编程，并不是为了取代 `java.util.concurrent`。

结构化并发将不同线程中运行的多个任务视为单个工作单元，从而简化错误处理、提高可靠性并增强可观察性。也就是说，结构化并发保留了单线程代码的可读性、可维护性和可观察性。

结构化并发的基本 API 是 [`StructuredTaskScope`](https://openjdk.org/jeps/480)。`StructuredTaskScope` 支持将任务拆分为多个并发子任务，在它们自己的线程中执行，并且子任务必须在主任务继续之前完成。
...
关于这个新特性的详细介绍，可以看看[Java 21 新特性概览(重要)](./Java 21 新特性概览(重要).md)这篇文章中的介绍。
...
JDK 24 于 2025 年 3 月发布，这是一个非 LTS（长期支持版）版本。它的下一个版本是 2025 年 9 月发布的 LTS 版本 **JDK 25**。

JDK 24 共有 24 个新特性，这篇文章会挑选其中较为重要的一些新特性进行详细介绍：

- [JEP 478: Key Derivation Function API (密钥派生函数 API, 预览)](https://openjdk.org/jeps/478)
...
## JEP 478: Key Derivation Function API（密钥派生函数 API，预览）

密钥派生函数 API 提供了一套用于从初始密钥和其他数据派生额外密钥的标准接口。它可以为不同的加密目的（如加密、认证等）生成多个不同的密钥，避免直接重复使用同一密钥。该 API 在 JDK 24 中处于预览阶段。

通过该 API，开发者可以使用 HKDF 等由安全提供者实现的密钥派生算法：
...
在传统 JVM 中，应用在每次启动时需要动态加载和链接类。该特性通过缓存已加载和链接的类，减少后续启动中的重复工作。JEP 483 给出的 Spring PetClinic 基准中，使用 AOT 缓存后启动时间最多缩短了约 42%；实际收益取决于应用和运行环境。

这个优化不要求修改应用程序、库或框架代码，但需要先记录训练运行并创建 AOT 缓存，随后在正式启动时加载该缓存。相关参数包括 `-XX:AOTMode=record`、`-XX:AOTConfiguration`、`-XX:AOTMode=create` 和 `-XX:AOTCache`。
...
- JMOD 文件是模块化归档文件，可以包含类文件、原生库、配置文件、头文件、法律声明等内容。
...
JDK 19 以孵化器 API 的形式引入了结构化并发。JDK 24 中该 API 处于第四次预览阶段，目的是简化多线程编程，并不是为了取代 `java.util.concurrent`。
...
JDK 25 被 Oracle 等大多数 JDK 发行方列为 LTS（长期支持版）。Oracle 当前的 LTS 版本包括 JDK 8、JDK 11、JDK 17、JDK 21 和 JDK 25。
...
作用域值提供从调用者到被调用者的单向不可变数据传递。通过结构化并发创建的子线程可以继承父线程中的绑定，而且这种继承无需复制绑定，从而减少大量线程共享上下文时的时间和空间开销。
...
该特性在 [JEP 450](https://openjdk.org/jeps/450 "JEP 450")（JDK 24）中作为实验特性引入，JDK 25 转为正式特性。
...
Shenandoah GC 在 JDK 12 中作为实验特性引入，并在 JDK 15 中转为正式产品特性。它默认关闭，可通过 `-XX:+UseShenandoahGC` 启用。
...
JDK 19 以孵化器 API 的形式引入了结构化并发。JDK 25 中该 API 处于第五次预览阶段，目的是简化多线程编程，并不是为了取代 `java.util.concurrent`。
...
- **消除队头阻塞**：HTTP/2 多请求复用一个 TCP 连接，一旦发生丢包，就会阻塞住所有的 HTTP 请求。由于 QUIC 协议的特性，HTTP/3 在一定程度上解决了队头阻塞（Head-of-Line blocking, 简写：HOL blocking）问题，一个连接建立多个不同的数据流，某个数据流发生丢包时，其他数据流不受影响。
...
详细介绍可以阅读这篇文章：[计算机网络常见面试题总结（上）](https://javaguide.cn/计算机基础/计算机网络/other-network-questions.html)（网络分层模型、常见网路协议总结、HTTP、WebSocket、DNS 等）

**使用方式**：

JDK 26 中 `HttpClient` 的默认协议仍然是 HTTP/2。要使用 HTTP/3，需要在 `HttpClient` 或单个 `HttpRequest` 上显式指定 `HTTP_3`：
...
从 JDK 5 开始，为支持序列化等场景，反射 API 允许通过**深度反射**修改部分 `final` 字段：
...
该特性在 JDK 25 中由 [JEP 502: Stable Values（预览）](https://openjdk.org/jeps/502) 首次预览；JDK 26 的 JEP 526 将 API 更名为 Lazy Constants，并进行第二次预览。

传统的 `static final` 字段通常在类初始化时就会初始化，这会：
...
JDK 19 以孵化器 API 的形式引入了结构化并发。JDK 26 中该 API 处于第六次预览阶段，目的是简化多线程编程，并不是为了取代 `java.util.concurrent`。
...
- [《Java 9 Modules: part 1》](https://stacktraceguru.com/Java 9 新特性概览/module-introduction)
