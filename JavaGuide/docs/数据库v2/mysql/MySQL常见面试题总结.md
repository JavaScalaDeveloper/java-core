...
关于两者的详细对比，请参考我写的 [MySQL 时间类型数据存储建议](./MySQL日期类型选择建议.md)。
...
MySQL 索引相关的问题比较多，对于面试和工作都比较重要，于是，我单独抽了一篇文章专门来总结 MySQL 索引相关的知识点和问题：[MySQL 索引详解](./MySQL索引详解.md) 。
...
读写分离和分库分表相关的问题比较多，于是，我单独写了一篇文章来介绍：[读写分离和分库分表详解](../../高性能/read-and-write-separation-and-library-subtable.md)。
...
[数据冷热分离详解](../../高性能/data-cold-hot-separation.md)
...
在面试中，建议按优先级依次介绍慢 SQL 定位、[索引优化](./MySQL索引详解.md)、表结构设计和 [SQL 优化](../../高性能/sql-optimization.md)等内容。架构层面的优化，如[读写分离和分库分表](../../高性能/read-and-write-separation-and-library-subtable.md)、[数据冷热分离](../../高性能/data-cold-hot-separation.md) 应作为最后的手段，除非在特定场景下有明显的性能瓶颈，否则不应轻易使用，因其引入的复杂性会带来额外的维护成本。
