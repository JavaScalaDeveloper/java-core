...
默认情况下，内容样式由编辑器JavaScript加载，这使得它们只在用户编辑其内容时出现，而这通常在应用程序的后端发生。如果您想在前端使用相同的样式，您可能会发现自己处于这样一种情况，需要您为此加载CKEditor，这(就性能而言)不是最好的主意。为了避免在你的前端不必要的依赖，使用一个完整的CKEditor 5内容样式列表样式表所有编辑器功能。有两种方法可以获得它
...
但是官方的CSS样式，对于h1,h2等标签没有很好的处理，因此我们再次加入自己的自定义css进去
