...
![Redis图标](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-96e079f9-49a3-4c55-b0a4-47d043732b62.png)
...
![Redis](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-b02e44b3-3299-450f-b767-4a862b5ac8ff.png)
...
![Redis缓存](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-d44c2397-5994-452f-8b7b-eb85d2b87685.png)
...
![Redis基本数据结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-10434dc7-c7a3-4c1a-b484-de3fb37669ee.png)
Redis有五种基本数据结构。
...
![Redis使用IO多路复用和自身事件模型](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e05bca61-4600-495c-b92a-25ac822e034e.png)
...
![多路复用模型](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-eb541432-d68a-4dd9-b427-96c4dd607d64.png)
...
![官方单线程解释](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-344b8461-98d4-495b-a697-70275b0abad6.png)
...
![Redis6.0多线程](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-b7b24e25-d2dc-4457-994f-95bdb3674b8e.png)
...
![Redis持久化两种方式](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-3bda4a46-adc3-4f0d-a135-b8ae5d4c0d5d.png)
...
![save和bgsave](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-ffe56e32-34c5-453d-8859-c2febbe6a038.png)
...
![AOF工作流程](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-a9fb6202-b1a1-484d-a4fa-fef519090b44.png)流程如下：
...
![Redis启动加载数据](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-f9aab5e9-a875-4316-9ec9-0c5650afe5c1.png)
...
![混合持久化](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-19c531e5-da95-495a-a4c4-d63a0b8bba95.png)
...
![Redis主从复制简图](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-60497f1e-8afb-44b3-bb7a-d4c29e5ac484.png)
...
![一主一从结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-5d91a67c-dbff-4a8d-bf9d-1fe7602d5a27.png)
2.一主多从结构

一主多从结构（又称为星形拓扑结构）使得应用端可以利用多个从节点实现读写分离（见图6-5）。对于读占比较大的场景，可以把读命令发送到从节点来分担主节点压力。

![一主多从结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-71074254-699a-480b-bbb0-c68f364a380b.png)
3.树状主从结构 

树状主从结构（又称为树状拓扑结构）使得从节点不但可以复制主节点数据，同时可以作为其他从节点的主节点继续向下层复制。通过引入复制中间层，可以有效降低主节点负载和需要传送给从节点的数据量。

![树状主从结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-dff14203-5e01-4d1b-a775-10ee444ada54.png)
### 15.Redis的主从复制原理了解吗？
Redis主从复制的工作流程大概可以分为如下几步：

![Redis主从复制工作流程](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-21123b1e-68b4-436b-ac84-3365a49a81bd.png)
1.  保存主节点（master）信息
...
![主从数据同步方式](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-7518f715-6dee-4e70-b972-8aed9879e451.png)
...
![全量复制](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-aa8d2960-b341-49cc-b04c-201241fd15de.png)
...
![部分复制](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-87600c72-cc6a-4656-81b2-e71864c97f23.png)
...
![Redis Sentinel](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-8b1a055c-f077-49ff-9432-c194d4fc3639.png)
...
![Redis Sentinel工作流程](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-4074d72a-886a-4892-8f55-80112005aad8.png)

- **定时监控**
  

![三个定时任务](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e7708f8d-ef34-4255-b5d0-cb300c649716.png)Redis Sentinel通过三个定时监控任务完成对各个节点发现和监控：
...
![主观下线和客观下线](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-11839a24-9249-48a5-8c9d-888aa80d91dc.png)
...
![故障转移](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-0618a5e2-e94f-40d7-888a-e78019ba8f93.png)
...
![领导者Sentinel节点选举](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-6586ae3c-6b33-4c9f-8137-f02fc0e6cfae.png)
...
![新的主节点](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-03976d35-20b6-4efe-aa9c-7d3759460d34.png)
...
![Redis 集群示意图](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-5cbc6009-251e-4d5b-8f22-8d543938eccb.png)
...
![分布式数据分区](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-ceb49e41-dfd7-4d1e-91f9-c299437227d2.png)
...
![节点取余分区](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-8b1fcaec-37e6-420a-9ca2-03615232af17.png)
...
![一致性哈希分区](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-89bd1c1c-251c-4f53-bba3-fe945b2ae9e2.png)
...
![虚拟槽分配](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e0ed9d62-3406-40db-8b01-c931f1020612.png)
...
![集群创建](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-046a512c-baab-4e3a-9409-2af58088cceb.png)

**设置节点**
Redis集群一般由多个节点组成，节点数量至少为6个才能保证组成完整高可用的集群。每个节点需要开启配置cluster-enabled yes，让Redis运行在集群模式下。

![节点和握手](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e6064ba6-fd6f-4270-92f9-68c0bb98fd4b.png)
**节点握手**
...
![分配槽](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-15341792-e7a6-428c-a109-22827e02be5f.png)
##### 故障转移
...
![主观下线](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-84a2a89e-f9ea-4681-b748-1a4f1dee172b.png)
当某个节点判断另一个节点主观下线后，相应的节点状态会跟随消息在集群内传播。通过Gossip消息传播，集群内节点不断收集到故障节点的下线报告。当 半数以上持有槽的主节点都标记某个节点是主观下线时。触发客观下线流程。

![主观下线和客观下线](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-b61a6109-7aea-45ab-a53c-267eebb9180a.png)
...
![故障恢复流程](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-0e5a49b3-cb5a-4aef-a81f-fce50a012a39.png)
1. 资格检查
...
![选举投票](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-d0e16ea3-6683-43f4-82a3-80478703ae06.png)
5. 替换主节点
...
![集群的伸缩](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-dd3e9494-eddb-4861-85f7-2646018d93f6.png)其实，集群扩容和缩容的关键点，就在于槽和节点的对应关系，扩容和缩容就是将一部分`槽`和`数据`迁移给新节点。

例如下面一个集群，每个节点对应若干个槽，每个槽对应一定的数据，如果希望加入1个节点希望实现集群扩容时，需要通过相关命令把一部分槽和内容迁移给新节点。

![扩容实例](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-1d24bb63-2b05-4db9-bd6b-983f16a4830e.png)缩容也是类似，先把槽和数据迁移到其它节点，再把对应的节点下线。
...
![缓存击穿](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-86579ee6-9dae-4274-a5cc-af6812f48da4.png)
解决⽅案：

1. 加锁更新，⽐如请求查询A，发现缓存中没有，对A这个key加锁，同时去数据库查询数据，写⼊缓存，再返回给⽤户，这样后⾯的请求就可以从缓存中拿到数据了。

![加锁更新](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-cf63911a-8501-493e-a375-8b47a9f33358.png)
...
![缓存穿透](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-029951e6-8b99-4364-a570-010853deb594.png)
...
![缓存空值/默认值](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-288af5a2-ae5a-427a-95e9-b4a658b01386.png)
...
![布隆过滤器](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-0e18ea40-a2e5-4fa6-989e-e771f6e4b0fc.png)
两种解决方案的对比：

![缓存空对象核布隆过滤器方案对比](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e8a382c9-4379-44ab-b1dc-fb598a228105.png)
...
![缓存雪崩](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-1464fe22-c463-4850-8989-b899510cb10e.png)
...
![布隆过滤器](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-d0b8d85c-85dc-4843-b4be-d5d48338a44e.png)我们判断缓存key是否存在，同样，K个哈希函数，映射到bit列表上的K个点，判断是不是1：
...
![删除缓存和更新缓存](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-ebad0a67-3012-4466-a4dc-e834104c48f8.png)
...
![先更数据库还是先删缓存](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-5c929a9e-a723-43b3-8f3c-f22c83765f9d.png)目前最流行的缓存读写策略cache-aside-pattern就是采用先更数据库，再删缓存的方式。
...
![缓存一致性](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-51b3453e-e1f1-4c65-9879-2aa59df23eca.png)
...
![消息队列保证key被删除](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e4a61193-515a-409f-a436-2733abc3a86e.png)这种方案看起来不错，缺点是对业务代码有一定的侵入性。
...
![数据库订阅+消息队列保证key被删除](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-37c07418-9cd8-43d9-90e7-0cb43b329025.png)
...
![延时双删](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-fab24753-9c53-4432-9413-5feba07ae1e3.png)
...
![延时双删](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-6d4ab7e6-8337-4576-bbf0-79202a1c3331.png)
...
![本地缓存/分布式缓存保持一致](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-20c15f0d-fb3c-4922-94b1-edcd856658be.png)
...
![热key处理](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-6fa972ec-5531-48f2-a608-4465d79d4518.png)
...
![在这里插入图片描述](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-de726c67-e56f-4f4b-96be-5cb998659541.png)
...
![Redis六种内存溢出控制策略](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-5be7405c-ee11-4d2b-bea4-9598f10a1b17.png)
...
![Redis阻塞排查](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e6a35258-7a78-4489-90b7-e47a4190802b.png)
...
![大key处理](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e4aaafda-fce1-47f0-8b2b-7261d47b720b.png)
...
![list作为队列](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e4b192a1-3ba7-4f4e-98de-e93f437cff7c.png)
...
![list作为队列，brpop](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e9581e51-ffc8-4326-9af4-07816743dc88.png)
...
![pub/sub](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-bc6d05be-3701-4e23-b4ca-6330c949f020.png)
...
![zset实现延时队列](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-54bbcc36-0b00-4142-a6eb-bf2ef48c2213.png)
...
![Redis事务](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-2ed7ae21-16a6-4716-ac89-117a8c76d3db.png)
...
![Pipelining示意图`](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-38aee4c1-efd2-495e-8a6d-164d21a129b1.png)
...
![setnx(set if not exists)](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-a5bddceb-66f6-4965-9f16-05d7179697fc.png)
...
![锁超时释放](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-2d3de973-c910-4efe-93e9-cfd50f84c91a.png)
````java
> setnx lock:fighter true
OK
...
![set原子指令](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-710cdd19-98ea-4e96-b579-ff1ebb0d5de9.png)
...
![redisObject对应的映射](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-a1b2d2f9-6895-4749-9bda-9314f08bca68.png)
我们常用的数据类型和编码对应的映射关系：


![类型-编码-结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-7cf91aa9-8db5-4abe-803e-a9e8f3bcb9e4.png)
...
![SDS](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-7c038f2c-b5ee-4229-9449-713fab3b1855.png)
...
![链表linkedlist](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-1adef9c0-8feb-4836-8997-84bda96e2498.png)
...
![字典](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-9934b4a2-c253-4d42-acf4-c6c940840779.png)
...
![跳跃表](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-886ee2a8-fb02-4908-bbba-d4ad2a211094.png)
6. **整数集合intset**：⽤于保存整数值的集合抽象数据结构，不会出现重复元素，底层实现为数组。

![整数集合intset](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-833dbfb2-7c79-4e7b-a143-8a4a2936cdd8.png)

7. **压缩列表ziplist**：压缩列表是为节约内存⽽开发的顺序性数据结构，它可以包含任意多个节点，每个节点可以保存⼀个字节数组或者整数值。


![压缩列表组成](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-99bcbe82-1d91-41bf-8900-a240856071f5.png)
...
![C语言的字符串](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-2541fd26-4e84-467d-8d8c-c731154a85d7.png)
...
![Redis sds](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-fc26a4e7-1c8d-4e82-b7f8-1f6b43d16d38.png)
...
![Redis字典结构](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-e08347a6-efd5-47c0-9adb-23baff82dbbd.png)
...
![跳跃表](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-08391728-5ba8-42a0-a287-9284451e0ee7.png)
...
![通过前进指针遍历](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-b153f782-e2e5-4f98-b251-04f06e16c073.png)
- **跨度**
...
![计算节点的排位](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-d2395b7e-2f31-4ca8-b06d-2cb47afaeb74.png)
- **分值和成员**
...
![压缩列表组成部分](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-6be492f7-9f92-4607-a4c4-81a612a3d7bd.png)压缩列表由这么几部分组成：
...
![压缩列表示例](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-b5d224c2-53ee-40a3-9efc-2feb7dd3d7a8.png)
...
![quicklist](https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/侧边栏/三份/redis-3b9785b0-6573-4c2d-8b7d-d5d1be799e26.png)
...
- [面渣逆袭 Java SE 篇👍](https://tobebetterjavaer.com/侧边栏/三份/javase.html)
- [面渣逆袭 Java 集合框架篇👍](https://tobebetterjavaer.com/侧边栏/三份/javathread.html)
- [面渣逆袭 Java 并发编程篇👍](https://tobebetterjavaer.com/侧边栏/三份/collection.html)
- [面渣逆袭 JVM 篇👍](https://tobebetterjavaer.com/侧边栏/三份/jvm.html)
- [面渣逆袭 Spring 篇👍](https://tobebetterjavaer.com/侧边栏/三份/spring.html)
- [面渣逆袭 Redis 篇👍](https://tobebetterjavaer.com/侧边栏/三份/redis.html)
- [面渣逆袭 MyBatis 篇👍](https://tobebetterjavaer.com/侧边栏/三份/mybatis.html)
- [面渣逆袭 MySQL 篇👍](https://tobebetterjavaer.com/侧边栏/三份/mysql.html)
- [面渣逆袭操作系统篇👍](https://tobebetterjavaer.com/侧边栏/三份/os.html)
- [面渣逆袭计算机网络篇👍](https://tobebetterjavaer.com/侧边栏/三份/network.html)
- [面渣逆袭RocketMQ篇👍](https://tobebetterjavaer.com/侧边栏/三份/rocketmq.html)
